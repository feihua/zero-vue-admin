...
<script>
  import { createFlashPromotion, deleteFlashPromotion, queryFlashPromotionList, updateFlashPromotion } from '@/api/sms/flash_promotion'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryFlashPromotionList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createFlashPromotion(this.temp).then(() => {
              this.getList()
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateFlashPromotion(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteFlashPromotion(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
