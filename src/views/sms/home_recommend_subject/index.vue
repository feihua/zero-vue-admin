  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="推荐状态" >
        <template slot-scope="scope">
          <span>{{ scope.row.recommend_status }}</span>
        </template>
      </el-table-column>
...
<script>
  import { createHomeRecommendSubject, deleteHomeRecommendSubject, queryHomeRecommendSubjectList, updateHomeRecommendSubject } from '@/api/sms/home_recommend_subject'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryHomeRecommendSubjectList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createHomeRecommendSubject(this.temp).then(() => {
              this.getList()
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateHomeRecommendSubject(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteHomeRecommendSubject(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
