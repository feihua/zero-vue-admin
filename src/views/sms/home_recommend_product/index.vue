...
<script>
  import { createHomeRecommendProduct, deleteHomeRecommendProduct, queryHomeRecommendProductList, updateHomeRecommendProduct } from '@/api/sms/home_recommend_product'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryHomeRecommendProductList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createHomeRecommendProduct(this.temp).then(() => {
              this.getList()
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateHomeRecommendProduct(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteHomeRecommendProduct(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
