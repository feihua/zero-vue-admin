...
<script>
  import { createHomeAdvertise, deleteHomeAdvertise, queryHomeAdvertiseList, updateHomeAdvertise } from '@/api/sms/home_advertise'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryHomeAdvertiseList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createHomeAdvertise(this.temp).then(() => {
              this.getList()
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateHomeAdvertise(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteHomeAdvertise(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
