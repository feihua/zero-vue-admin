...
<script>
  import { createHomeBrand, deleteHomeBrand, queryHomeBrandList, updateHomeBrand } from '@/api/sms/home_brand'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryHomeBrandList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createHomeBrand(this.temp).then(() => {
              this.getList()
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteHomeBrand(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
