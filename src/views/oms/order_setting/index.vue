  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="秒杀订单超时关闭时间">
        <template slot-scope="scope">
          {{ scope.row.flashOrderOvertime }}
        </template>
      </el-table-column>
      <el-table-column label="正常订单超时时间" >
        <template slot-scope="scope">
          <span>{{ scope.row.normalOrderOvertime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="发货后自动确认收货时间">
        <template slot-scope="scope">
          {{ scope.row.confirmOvertime }}
        </template>
      </el-table-column>
      <el-table-column label="自动完成交易时间">
        <template slot-scope="scope">
          {{ scope.row.finishOvertime }}
        </template>
      </el-table-column>
      <el-table-column label="订单完成后自动好评时间">
        <template slot-scope="scope">
          {{ scope.row.commentOvertime }}
        </template>
      </el-table-column>
...
<script>
  import { createOrderSetting, deleteOrderSetting, queryOrderSettingList, updateOrderSetting } from '@/api/oms/order_setting'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryOrderSettingList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createOrderSetting(this.temp).then(() => {
              this.getList()
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateOrderSetting(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteOrderSetting(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
