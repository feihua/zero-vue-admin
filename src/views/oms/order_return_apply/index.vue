...
<script>
  import { createOrderReturnApply, deleteOrderReturnApply, queryOrderReturnApplyList, updateOrderReturnApply } from '@/api/oms/order_return_apply'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryOrderReturnApplyList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createOrderReturnApply(this.temp).then(() => {
              this.getList()
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateOrderReturnApply(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteOrderReturnApply(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
