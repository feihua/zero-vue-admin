  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="剩余抽奖次数">
        <template slot-scope="scope">
          {{ scope.row.luckeyCount }}
        </template>
      </el-table-column>
      <el-table-column label="历史积分数量">
        <template slot-scope="scope">
          {{ scope.row.historyIntegration }}
        </template>
      </el-table-column>
...
<script>
  import { createMember, deleteMember, queryMemberList, updateMember } from '@/api/ums/member/index'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryMemberList(this.listQuery).then(response => {
