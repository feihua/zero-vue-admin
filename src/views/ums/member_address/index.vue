...
<script>
  import { createMemberAddress, deleteMemberAddress, queryMemberAddressList, updateMemberAddress } from '@/api/ums/member_address/index'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryMemberAddressList(this.listQuery).then(response => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createMemberAddress(this.temp).then(() => {
              this.getList()
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateMemberAddress(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteMemberAddress(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
